...
class SimMIMMaskGenerator(BaseTransform):
    ...
    def __init__(self,
        ...
        self.mask_count = int(np.ceil(self.token_count * self.mask_ratio)) #ceil 取整
...
@TRANSFORMS.register_module()
class SimMIMMaskGenerator1(BaseTransform):
    """Generate random block mask for each Image.
...
@TRANSFORMS.register_module()
class RandomBlockErasing(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        num_erased_blocks = int(num_blocks_x * num_blocks_y * self.erase_ratio)

        for _ in range(num_erased_blocks):
            block_x = np.random.randint(num_blocks_x)
            block_y = np.random.randint(num_blocks_y)

            left = block_x * self.block_size
            upper = block_y * self.block_size
            right = left + self.block_size
            lower = upper + self.block_size

            # Create a black block to erase the region
            img[upper:lower, left:right] = 0

        # Put the modified image back into the results dictionary
        results['img'] = img
    ...
    def __repr__(self) -> str:
        ...
        repr_str += f'erase_ratio = {self.erase_ratio})'
        return repr_str
