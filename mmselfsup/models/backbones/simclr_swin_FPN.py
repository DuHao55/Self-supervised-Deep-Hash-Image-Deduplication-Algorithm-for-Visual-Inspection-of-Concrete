...
class SimCLRSwinTransformer_FPN(SwinTransformer):
    ...
    def __init__(self,
        ...
        # self.mask_constant = torch.zeros(1, 1, self.embed_dims)
    ...
    def forward(self, x: torch.Tensor,
        ...
        # mask_constant = self.mask_constant.expand(B, L, -1).to('cuda')  #-1表示当前维度不变，mask_token reshape （B,L,embed_dims),same as x(patch_embed)
        #
        # w = mask.flatten(1).unsqueeze(-1).type_as(mask_constant)  #mask=(b,h,w)->(b,h*w)->(b,h*w,1)  w=(8,4096,1),4096=64*64
        # x = x * (1. - w) + mask_constant * w  # x=(8,4096,96)  w type_as mask_token,also can trainable,
    ...
    def _frozen_parameters (self) -> None:
