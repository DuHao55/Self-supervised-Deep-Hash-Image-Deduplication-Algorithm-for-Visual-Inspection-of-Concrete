...
class SimMIM(BaseModel):
    ...
    def extract_feat(self, inputs: List[torch.Tensor],
        ...
        feat = self.neck(img_latent[0]) #img_latent[0]
        self.mask = mask
        return feat
    ...
    def reconstruct(self,
        ...
        """
        pred = torch.einsum('nchw->nhwc', features).detach().cpu() #detach返回一个新的tensor，从当前计算图中分离下来的，但是仍指向原变量的存放位置,不同之处只是requires_grad为false，得到的这个tensor永远不需要计算其梯度，不具有grad。

        # transform patch mask to pixel mask
        mask = self.mask.detach()
        p1 = int(self.backbone.patch_embed.init_input_size[0] //#192/（192/4）=4
                 self.backbone.patch_resolution[0])
        p2 = int(self.backbone.patch_embed.init_input_size[1] //#192/（192/4）=4
                 self.backbone.patch_resolution[1])
        mask = mask.repeat_interleave(       #掩码重复（repeat_interleave），以匹配补丁大小重复张量的元素,p1为每个元素的重复次数
